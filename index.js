...
(function () {
    ...
    const tianyi = () => {
        ...
        $('.file-list-ul .c-file-item[data-isfolder!="true"]').each(function () {
            let li = $(this);
            if (!li || li.find(".sherer-fileid").length) { return; }
            let fileid = li.attr('data-fileid');
            debug && console.log(fileid);
            if (!fileid) { return; }
            li.find('.file-item').after(`<p class="sherer-fileid sherer-btn sherer-font sherer-center sherer-m-5" data-content="tianyi_${fileid}">${tc('copy_url')}</p>`);
        });
    };
